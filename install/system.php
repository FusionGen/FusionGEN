...
class Install
{
    ...
    {
        $req    = ['mysqli', 'curl', 'openssl', 'soap', 'gd', 'mbstring', 'json'];
    ...
    private function config()
    {
        ...
        if (in_array($_POST['emulator'], ['arcemu', 'summitemu', 'ascemu'])) {
            switch ($_POST['expansion']) {
                case "wotlk":
                    $config->set('disabled_expansions', [32]);
                    break;

                case "tbc":
                    $config->set('disabled_expansions', [24, 32]);
                    break;

                case "vanilla":
                    $config->set('disabled_expansions', [8, 24, 32]);
                    break;
            }
        } else {
            switch ($_POST['expansion']) {
                case "legion":
                    $config->set('disabled_expansions', [7]);
                    break;

                case "wod":
                    $config->set('disabled_expansions', [6, 7]);
                    break;

                case "mop":
                    $config->set('disabled_expansions', [5, 6, 7]);
                    break;

                case "cata":
                    $config->set('disabled_expansions', [4, 5, 6, 7]);
                    break;

                case "wotlk":
                    $config->set('disabled_expansions', [3, 4, 5, 6, 7]);
                    break;

                case "tbc":
                    $config->set('disabled_expansions', [2, 3, 4, 5, 6, 7]);
                    break;

                case "vanilla":
                    $config->set('disabled_expansions', [1, 2, 3, 4, 5, 6, 7]);
                    break;

                default:
                    $config->set('disabled_expansions', []);
                    break;
            }
        }
