...
class Characters_model
{
    ...
    public function getOnlinePlayers()
    {
        return $this->getCharacters(
            columns(
                "characters",
                ["guid", "account", "name", "race", "class", "gender", "level", "zone"],
                $this->realmId
            ),
            [
    ...
    public function getOnlineCount($faction = false)
    {
        ...
        switch ($faction) {
            default:
                $query = $this->db->query(
                    "SELECT COUNT(*) as `total` FROM " . table("characters", $this->realmId) . " WHERE " . column("characters", "online", false, $this->realmId) . "='1'"
                );
                break;

            case "alliance":
                $query = $this->db->query(
                    "SELECT COUNT(*) as `total` FROM " . table("characters", $this->realmId) . " WHERE " . column("characters", "online", false, $this->realmId) . "='1' AND " . column("characters", "race") . " IN(" . implode(",", get_instance()->realms->getAllianceRaces()) . ")"
                );
                break;

            case "horde":
                $query = $this->db->query(
                    "SELECT COUNT(*) as `total` FROM " . table("characters", $this->realmId) . " WHERE " . column("characters", "online", false, $this->realmId) . "='1' AND " . column("characters", "race") . " IN(" . implode(",", get_instance()->realms->getHordeRaces()) . ")"
                );
                break;
        }
    ...
    public function getCharactersByAccount($acc = false)
    {
        ...
        return $this->getCharacters(
            columns(
                "characters",
                ["guid", "name", "race", "class", "gender", "level", "online", "money"],
                $this->realmId
            ),
            [
                column("characters", "account", false, $this->realmId) => $acc,
            ]
        );
    }
