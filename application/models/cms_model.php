...
class Cms_model extends CI_Model
{
    ...
    private function logVisit()
    {
        if (!$this->input->is_ajax_request() && !isset($_GET['is_json_ajax'])) {
            $this->db->query("INSERT INTO visitor_log(`date`, `ip`) VALUES(?, ?)", [date("Y-m-d"), $this->input->ip_address()]);
        }
    ...
    {
        $this->db->query("DELETE FROM ci_sessions WHERE last_activity < ?", [time() - 60 * 60]);
    }
    ...
    {
        $this->db->select('COUNT(*) as `total`')->from('private_message')->where(['user_id' => $this->user->getId(), 'read' => 0]);
        $query = $this->db->get();
