...
class External_account_model extends CI_Model
{
    ...
    public function initialize($where = false)
    {
        ...
        if (!$where) {
            $query = $this->connection->query(query("get_account_id"), [$this->session->userdata('id')]);
        } else {
    ...
    public function createAccount($username, $password, $email, $expansion, $isHashed = false)
    {
        ...
        if ($expansion > 4) {
            ...
            //$this->connection->update(table("account"), [column("account", "battlenet_account") => $userId, column("account", "battlenet_index") => 1]);
