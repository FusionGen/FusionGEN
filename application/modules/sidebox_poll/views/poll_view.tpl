{if $poll}
	...
	{if $poll.answers}
		...
		{if !$myVote}
			<a id="poll_actions" class="nice_button rounded my-3" href="javascript:void(0)" onClick="Poll.toggle(this)">
				{lang("show_results", "sidebox_poll")}
			</a>
		{/if}
	{else}
		<div class="fw-bold text-center my-3">{lang("no_poll", "sidebox_poll")}</div>
	{/if}
{/if}
