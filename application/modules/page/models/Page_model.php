...
class Page_model extends CI_Model
{
    ...
    public function setPermission($id)
    {
        ...
        $this->db->query("INSERT INTO acl_roles(`name`, `module`) VALUES(?, '--PAGE--')", [$id]);
        $this->db->query("INSERT INTO acl_roles_permissions(`role_name`, `permission_name`, `module`, `value`) VALUES(?, ?, '--PAGE--', 1)", [$id, $id]);
    }

    public function deletePermission($id)
    {
        $this->db->query("UPDATE pages SET `permission`='' WHERE id=?", [$id]);
        $this->db->query("DELETE FROM acl_roles WHERE module='--PAGE--' AND name=?", [$id]);
    }
