			...
			{/if}
				...
				<div class="col-md-12 {if $article.type == '0'}col-lg-12{else}col-lg-8{/if}">
					<div class="article-head">
						...
						<div class="article-metadata">{lang("posted_by", "news")} <a href="{$url}profile/{$article.author_id}" data-tip="{lang('view_profile', 'news')}">{$article.author}</a>, {lang("on", "news")} {$article.date} {if $is_single && $article.comments != -1} - <a {$article.link} {$article.comments_button_id}>{lang("comments", "news")} {$article.comments}</a>{/if}</div>
					</div>
